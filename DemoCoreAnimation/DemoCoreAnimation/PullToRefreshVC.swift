...
class PullToRefreshVC: UIViewController {
    ...
    @objc func gestureAction(_ sender: UIPanGestureRecognizer) {
        ...
        switch sender.state {
            ...
            case .changed: guard AnimationSetting.isAnimation else { return }
                shapeLayer.path = createPath(point: point)
            case .ended, .failed, .cancelled: guard AnimationSetting.isAnimation else { return }
...
extension PullToRefreshVC: CAAnimationDelegate {
